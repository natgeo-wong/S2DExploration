...
ilon = argmin.(slon.-ggrd.lon)
ilat = argmin.(slat.-ggrd.lat)
...
fnc = joinpath(tbd.path,"SGP-Tb-$(Dates.format(tbd.start,dateformat"yyyymmdd"))-$(Dates.format(tbd.stop,dateformat"yyyymmdd")).nc")
...
ncvar = defVar(pds,"Tb",Float32,("valid_time",),attrib = Dict(
    "units"         => "K",
