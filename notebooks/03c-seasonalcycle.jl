...
md"
# 03c. Exploring the Seasonal Cycle
"
...
function load_climatology(armsite,e5ds,evar::SingleLevel)
...
function load_climatology(armsite,e5ds,evar::PressureLevel)
    ...
    for imo = 1 : 12
        ii = month.(dt) .== imo
        pre[:,imo] = dropdims(mean(pds[evar.ncID][:,ii],dims=2),dims=2)
    end
    close(pds)
...
begin
    sgl = load_climatology(armsite,e5ds,esgl)
    md"Loading Single-Level Variable: $(esgl.ID)"
end

# ╔═╡ 2b99c24a-75bc-4f5b-9171-a6d297d74722
begin
    lvl,pre = load_climatology(armsite,e5ds,epre)
...
begin
    ...
    ax1_1 = Axis(
        ...
        xticklabelsvisible=false,xticks=1:12
        # yscale=log10,yticks=[10,100,1000]
    )

    c1 = heatmap!(ax1_1,1:12,lvl,pre'.*1e4,colorrange=(-1,1).*1e-1,colormap=:RdBu)
    xlims!(ax1_1,0.5,12.5)
    ylims!(ax1_1,1000,10)

    # axislegend()
    
    ax1_2 = Axis(f1[2,1],width=500,height=50,xlabel="Local Time",xticks=1:12)

    lines!(ax1_2,1:12,sgl)
    xlims!(ax1_2,0.5,12.5)
